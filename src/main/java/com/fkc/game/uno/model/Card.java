...
public class Card {
    ...
    /**
     * The special Unicode character mapped to this card's ItemsAdder font
     * image (see uno_cards_config.yml). Codepoints start at U+F500 —
     * deliberately far from mahjong's U+F400-F429 range so the two never
     * collide even though both share the same server/resourcepack.
     *
     * Layout: each color gets a contiguous block of 13 codepoints
     * (numbers 0-9, then Skip, Reverse, Draw Two), followed by Wild and
     * Wild Draw Four at the very end.
     */
    public String imageGlyph() {
        int base = switch (color) {
            case RED -> 0xF500;
            case YELLOW -> 0xF500 + 13;
            case GREEN -> 0xF500 + 26;
            case BLUE -> 0xF500 + 39;
            case WILD -> 0xF500 + 52; // only WILD/WILD_DRAW_FOUR reach here
        };
