...
public class TaiwanHandEvaluator {
    ...
    /**
     * Structural validity only — is this a legal 5-sets+pair shape,
     * regardless of whether any tai would apply? (Taiwanese mode doesn't
     * gate "valid" on having at least one tai the way Riichi gates on
     * having at least one yaku, but this explicit shape-only check is
     * still useful on its own for tenpai detection.)
     */
    ...
    /**
     * Given a "waiting" concealed hand (size == 16 - 3*melds.size()),
     * returns every tile type that would complete it. Empty if not tenpai.
     */
    public static List<Tile> tenpaiWaits(List<Tile> concealed, List<Meld> melds) {
        ...
        for (int type = 0; type < 34; type++) {
            ...
            test.add(candidate);
            if (hasValidShape(test, melds)) waits.add(candidate);
        }
        return waits;
    }
    ...
    private static WinResult score(Decomposition d, List<Meld> melds, List<Tile> flowers, WinContext ctx, TaiSettings s) {
        ...
        result.points = (long) (ctx.basePoints + totalTai * ctx.pointsPerTai) * (ctx.isDealer ? ctx.dealerMultiplier : 1);
        return result;
    }
