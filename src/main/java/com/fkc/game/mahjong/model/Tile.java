...
public class Tile implements Comparable<Tile> {
     ...
     *
     * Codepoints intentionally start at U+F400, not U+E000. U+E000 is a very
     * commonly used Private Use Area start point for other resource packs /
     * addons (rank icons, emoji packs, etc.), and this server has a large
     * number of third-party ItemsAdder content packs installed — a codepoint
     * collision with one of them was the likely cause of tiles rendering as
     * the wrong (and unscaled/giant) image. U+F400 is far enough into the
     * PUA block that a collision is much less likely.
     */
    public String imageGlyph() {
        String symbol = switch (suit) {
            case MAN -> String.valueOf((char) (0xF400 + (number - 1)));
            case PIN -> String.valueOf((char) (0xF409 + (number - 1)));
            case SOU -> String.valueOf((char) (0xF412 + (number - 1)));
            case WIND -> String.valueOf((char) (0xF41B + (number - 1)));
            case DRAGON -> String.valueOf((char) (0xF41F + (number - 1)));
            case FLOWER_PLANT -> String.valueOf((char) (0xF422 + (number - 1)));
            case FLOWER_SEASON -> String.valueOf((char) (0xF426 + (number - 1)));
        };
        return symbol + (red ? "寶" : "");
    }
