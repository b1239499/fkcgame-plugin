...
public class TableManager {
    ...
    private final Leaderboard riichiLeaderboard;
    private final Leaderboard taiwanLeaderboard;
    private final com.fkc.game.core.PlayerPreferenceStore sidebarPrefs = new com.fkc.game.core.PlayerPreferenceStore();
    private final com.fkc.game.core.PlayerPreferenceStore tileDisplayPrefs = new com.fkc.game.core.PlayerPreferenceStore();
    ...
    public com.fkc.game.core.PlayerPreferenceStore tileDisplayPrefs() {
        return tileDisplayPrefs;
    }

    public TableManager(Plugin plugin) {
        this.plugin = plugin;
        this.riichiLeaderboard = new Leaderboard(plugin.getDataFolder(), "leaderboard-mahjong-riichi.properties");
        this.taiwanLeaderboard = new Leaderboard(plugin.getDataFolder(), "leaderboard-mahjong-taiwan.properties");
    }

    public Leaderboard leaderboardFor(com.fkc.game.mahjong.model.GameSettings.RuleSet ruleSet) {
        return ruleSet == com.fkc.game.mahjong.model.GameSettings.RuleSet.TAIWANESE ? taiwanLeaderboard : riichiLeaderboard;
    }

    public MahjongTable create(UUID hostUuid, String hostName, com.fkc.game.mahjong.model.GameSettings.RuleSet ruleSet) {
        MahjongTable table = new MahjongTable(plugin, hostUuid, leaderboardFor(ruleSet), sidebarPrefs);
        table.tileDisplayPrefs = tileDisplayPrefs;
        table.rewardStore = rewardStore;
        table.settings.ruleSet = ruleSet;
        table.addPlayer(hostUuid, hostName, false);
        table.toggleReady(hostUuid); // host defaults to already-ready — no need to /mahjong ready themselves
    ...
    private void applyConfigDefaults(MahjongTable table) {
        ...
        table.settings.taiwanBasePoints = config.getInt("mahjong.taiwan-rules.base-points", 0);
